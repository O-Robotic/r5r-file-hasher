...
void __fastcall Sha1_UpdateBlocks(unsigned int state[5], const unsigned char *data, size_t numBlocks);
...
void Sha1_Update(CSha1 *p, const unsigned char *data, size_t size)
{
  if (size == 0)
    return;
...
void Sha1_Final(CSha1 *p, unsigned char *digest)
{
  unsigned pos = (unsigned)p->count & 0x3F;
  ...
  */
  ...
  {
    unsigned __int64 numBits = (p->count << 3);
    SetBe32(p->buffer + 64 - 8, (unsigned int)(numBits >> 32));
    SetBe32(p->buffer + 64 - 4, (unsigned int)(numBits));
  }
...
void Sha1_PrepareBlock(const CSha1 *p, unsigned char *block, unsigned size)
{
  const unsigned __int64 numBits = (p->count + size) << 3;
  SetBe32(&((unsigned int*)(void *)block)[SHA1_NUM_BLOCK_WORDS - 2], (unsigned int)(numBits >> 32));
  SetBe32(&((unsigned int*)(void *)block)[SHA1_NUM_BLOCK_WORDS - 1], (unsigned int)(numBits));
  // SetBe32((UInt32 *)(block + size), 0x80000000);
  SetUi32((unsigned int*)(void *)(block + size), 0x80);
  size += 4;
  while (size != (SHA1_NUM_BLOCK_WORDS - 2) * 4)
  {
    *((unsigned int*)(void *)(block + size)) = 0;
    size += 4;
  }
}

void Sha1_GetBlockDigest(const CSha1 *p, const unsigned char *data, unsigned char *destDigest)
{
  __declspec(align(16))
  unsigned int st[SHA1_NUM_DIGEST_WORDS];
